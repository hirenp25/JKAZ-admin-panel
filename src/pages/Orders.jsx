            ...
            <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
              {filteredOrders.map((order) => (
                <tr key={order.id} className="hover:bg-gray-50 dark:hover:bg-gray-600 transition-colors">
                  ...
                  {/* 2. ORDER ID COLUMN (Clickable Link) */}
                  <td className="px-6 py-4 font-bold text-blue-600 dark:text-blue-400">
                    <Link to="/orders/details" className="hover:underline">
                      {order.id}
                    </Link>
                  </td>
                  ...
                  {/* 3. ACTION COLUMN (View Button) */}
                  <td className="px-6 py-4 text-right flex justify-end gap-2">
                    {/* View Details Button */}
                    <Link to="/orders/details" className="text-gray-500 hover:text-blue-600 dark:text-gray-400 dark:hover:text-blue-400 transition-colors" title="View Details">
                        <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" /></svg>
                    </Link>
                    
                    {/* Invoice Download Button */}
                    <button className="text-gray-500 hover:text-green-600 dark:text-gray-400 dark:hover:text-green-400 transition-colors" title="Download Invoice">
